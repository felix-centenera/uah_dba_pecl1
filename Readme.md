...
SELECT * FROM pg_stat_all_indexes WHERE indexrelname = 'idx_kilometros';

 SELECT * FROM bt_page_stats('idx_kilometros', 290);
SELECT * FROM bt_page_items('idx_kilometros', 17736);


indexrelname


SELECT relname AS index_name, relpages AS number_of_pages FROM pg_class WHERE relname = 'idx_kilometros';





SELECT oid::regclass AS tbl, relpages FROM   pg_class WHERE  relname = 'idx_kilometros';



SELECT * FROM bt_root('camiones3', 'idx_kilometros');


-- Selecciona la página raíz del índice B-tree
SELECT * FROM bt_root('nombre_de_la_tabla', 'nombre_del_indice');

-- Selecciona información sobre una página específica del índice B-tree (cambia el número de página según sea necesario)
SELECT * FROM bt_page_stats('nombre_de_la_tabla', 'nombre_del_indice', número_de_página);
